        ...
        foreach ($s1_category as $each_s1) {
          ...
          if(!empty($s2_category[0]->s2)){
            ...
            foreach($s2_category as $each_s2) {
              $s2class = str_replace(' ','',$each_s2->s2);
              $s2class = str_replace('"','',$s2class);
              $s2class = str_replace('/','',$s2class);
              $s2class = str_replace("'","",$s2class);
            ...
            // s2 is empty
            echo "<div class='custaccordion no-sub s1s2e-$s1class'><a href='".home_url()."/products/ps2/?m0=".urlencode($each_mc->m0)."&s1=".urlencode($each_s1->s1)."'>".$each_s1->s1."</a></div>";
          }
        }
        echo "</div>";  // end panel
      } else {
        // s1 is empty
        echo "<div class='custaccordion m0s1e-$m0class'><a href='".home_url()."/products/pm0/?m0=".urlencode($each_mc->m0)."'>".$each_mc->m0."</a></div>";
      }
    }
  echo "</div>";
