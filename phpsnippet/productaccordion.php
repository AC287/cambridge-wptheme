    ...
    if(!empty($s1_category[0]->s1)) {
      ...
      foreach ($s1_category as $each_s1) {
        ...
        if(!empty($s2_category[0]->s2)){
          ...
          echo "<div class='custaccordion'><a class='no-sub' href='".home_url()."/products/ps2/?m0=".urlencode($each_mc->m0)."&s1=".urlencode($each_s1->s1)."&s2=".urlencode($each_s2->s2)."'>".$each_s1->s1."</a></div>";
        }
      }
      echo "</div>";  // end panel
    } else {
