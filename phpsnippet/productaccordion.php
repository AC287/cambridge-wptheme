<?php
  ...
  foreach ($main_category as $each_mc) {
    ...
    $m0class = str_replace(' ','', $each_mc->m0);
    if(!empty($s1_category[0]->s1)) {
      $s1class = str_replace(' ','',$s1_category[0]->s1);
      ...
      foreach ($s1_category as $each_s1) {
        ...
        if(!empty($s2_category[0]->s2)){
          ...
          foreach($s2_category as $each_s2) {
            echo "<div class='custaccordion no-sub'><a href='".home_url()."/products/ps2/?m0=".urlencode($each_mc->m0)."&s1=".urlencode($each_s1->s1)."&s2=".urlencode($each_s2->s2)."'>".$each_s2->s2."</a></div>";
          }
          echo "</div>";  // end panel
        } else {
          // s2 is empty
          echo "<div class='custaccordion no-sub'><a href='".home_url()."/products/ps2/?m0=".urlencode($each_mc->m0)."&s1=".urlencode($each_s1->s1)."'>".$each_s1->s1."</a></div>";
        }
      }
      echo "</div>";  // end panel
    } else {
