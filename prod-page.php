						...
						for ($m0p = 0; $m0p < 3; $m0p++){
							...
							foreach ($main_category[$m0p] as $main_category2) {
								echo "<div class='group-container'>";
									...
									$s1_jcategory2 = $wpdb->get_results("SELECT DISTINCT s1 FROM wp_prodlegend WHERE jointcat = '$qm0';");
									...
									if(!empty($s1_category2[0]->s1)) {
										...
										foreach($s1_category2 as $s1_category2) {
											...
											$jqs1 = addslashes($s1_jcategory2->s1);	//Slash escape is required for special character such as " , ' , \ to search in query.
...
/*
						for($m0pi = 0; $m0pi < 3; $m0pi++) {	//sorting by m0
							// print_r($main_category[$m0pi]);
							...
							foreach ($main_category2 as $main_category2) {
								...
								echo "</div>";	// end group-container
							}	//end foreach.


					}	// end sorting by m0	for loop.
*/
/*
							$mPos = 0;
							foreach($main_category2 as $main_category2) {
